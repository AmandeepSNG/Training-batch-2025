...
function Wrapper () {
  ...
  useEffect(() => {
    const readTodoItems = async () => {
      const localData = JSON.parse(localStorage.getItem(keyName) || '[]') /**await(await fetch('https://jsonplaceholder.typicode.com/todos')).json()*/
  ...
  const markItemAsCompleted = (e: React.MouseEvent<HTMLDivElement>, item: listItem) => {
    // e => target ?
  }
  return (
    <div className='todo-wrapper'>
      ...
      <AddItemForm addItemToList={handleAddItemToList}/>
      {
        listItems.map((item: listItem, index: number) => (
          <div onClick={e => markItemAsCompleted(e, item)} className="todo-item" key={index}>
